...
class JessieOpenVLAExecutor(Node):
    """Node that handles skill execution on Jessie using OpenVLA.
    """
    ...
    def __init__(self, name='jessie_openvla_executor',
                 ...
                 openvla_model_path: str="/path/to/model"):
        """
        Keyword arguments:
        ...
        openvla_model_path: str -- Location from which to load the OpenVLA model
        ...
        self.load_model(openvla_model_path)
        ...
        while not self.check_preconditions_proxy.wait_for_service(timeout_sec=1.):
            self.get_logger().info(f'[openvla_executor] Service {precondition_checking_srv} not available, waiting...')
        self.precondition_checking_request = CheckSkillPreconditions.Request()

        self.running = True

    def load_model(self, model_path: str):
        """Sets self.model with an OpenVLA model loaded from model_path.

        Keyword arguments:
        model_path: str -- Location from which to load the OpenVLA model

        """
        self.input_processor = AutoProcessor.from_pretrained(model_path, trust_remote_code=True)
    ...
    def execute_skill(self):
        ...
        self.get_logger().info(f'[openvla_executor] Should check preconditions: {self.task.check_preconditions}')
        if self.task.check_preconditions and not self.preconditions_checked:
            self.get_logger().info(f'[openvla_executor] Checking preconditions for task {self.task.skill_description}')
            ...
            self.get_logger().info(f'[openvla_executor] Preconditions satisfied for task {self.task.skill_description}: {precondition_check_response.preconditions_satisfied}')
            if precondition_check_response.info:
                self.get_logger().info(f'[openvla_executor] Preconditions checking info: {precondition_check_response.info}')

            if not precondition_check_response.preconditions_satisfied:
                self.get_logger().error('[openvla_executor] Preconditions not satisfied; not executing')
        ...
        prompt = f'"In: What action should the robot take to {self.task.skill_description}?\nOut:"'
        inputs = self.input_processor(prompt, img).to("cuda:0", dtype=torch.bfloat16)
        ...
        if abs(gripper_action) < 1e-4 or np.linalg.norm(arm_vel_vector) < 0.005:
    ...
    def skill_execution_request_cb(self, skill_execution_request_msg: SkillExecutionRequest):
        ...
        * the requested model is not openvla
        * another skill is already being executed.
        ...
        if skill_execution_request_msg.model_name != 'openvla':
            self.get_logger().info(f'[openvla_executor] Ignoring skill execution request; model "{skill_execution_request_msg.model_name}" specified, but "openvla" expected')

        if self.skill_executing:
            self.get_logger().info('[openvla_executor] Ignoring skill execution request as another skill is already executing')

        self.task = skill_execution_request_msg
        self.get_logger().info(f'[openvla_executor] Received request for task {skill_execution_request_msg.skill_description}')
    ...
    def skill_execution_cancel_cb(self, msg: Empty):
        ...
        """
        if self.skill_executing:
            self.get_logger().error(f'[openvla_executor] Received a cancellation request while executing task {self.task.skill_description}')
            self.execution_status_msg.skill_name = self.task.skill_description
            self.execution_status_msg.status = ExecutionStatus.STOPPING
            ...
            self.skill_execution_status_pub.publish(self.execution_status_msg)
            self.stop_skill_execution()
            self.get_logger().error('[openvla_executor] Execution stopped')
        else:
            self.get_logger().error('[openvla_executor] No skill is currently executing; nothing to cancel')
...
def main(args=None):
    ...
    openvla_executor = JessieOpenVLAExecutor()

    rate = openvla_executor.create_rate(5, openvla_executor.get_clock())
    rate_thread = threading.Thread(target=rclpy.spin, args=(openvla_executor,), daemon=True)
    rate_thread.start()

    try:
        while rclpy.ok():
            if openvla_executor.skill_executing and openvla_executor.received_new_img:
                openvla_executor.execute_skill()
                openvla_executor.received_new_img = False
            rate.sleep()
    except Exception as exc:
        print(str(exc))

    print('Destroying OpenVLA execution node')
    openvla_executor.running = False
    time.sleep(0.5)
    openvla_executor.destroy_node()
    rclpy.shutdown()
